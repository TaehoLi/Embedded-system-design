   ...
   /* Check that MSP is the active stack */
    ...
    for (iter = 0; iter < niter; iter++) {
      /* Fill input vector */
	    ...
	    ioGetUint8((unsigned char*)string, 28 * 28 * 3, 5000);
	    for (ai_size i = 0; i < 28 * 28; i++) {
        ...
        for (ai_size i = 0; i < aiBufferSize(&ai_input[0]); ++i) {
	        ...
	        in_data[i] = (100.0f*(ai_float)(string[i][0] - 0x30) + 10.0f*(ai_float)(string[i][1] - 0x30) + (ai_float)(string[i][2] - 0x30)) / 255.0f;
        }
	    ...
	    recognized_digit = 0;
	    for (int i = 0; i < 10; i++) if (out_data[i] > out_data[recognized_digit]) recognized_digit = i;
