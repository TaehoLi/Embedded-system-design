...
//!	Generated Date	: Wed, 15, May 2019  
	File Path	: CanButtonTest/MontaVista/Move.cpp
*********************************************************************/
...
//## class TopLevel::Move
Move_C::Move_C(IOxfActive* theActiveContext) : upstate(true) {
...
bool Move_C::getUpstate() const {
    return upstate;
}

void Move_C::setUpstate(bool p_upstate) {
    upstate = p_upstate;
}

CanButton_C* Move_C::getItsCanButton() const {
    return itsCanButton;
}

void Move_C::setItsCanButton(CanButton_C* p_CanButton) {
    if(p_CanButton != NULL)
        {
            p_CanButton->_setItsMove(this);
        }
    _setItsCanButton(p_CanButton);
}

bool Move_C::startBehavior() {
    bool done = false;
    done = OMReactive::startBehavior();
    return done;
}

void Move_C::initStatechart() {
...
void Move_C::cleanUpRelations() {
    if(itsCanButton != NULL)
        {
            Move_C* p_Move = itsCanButton->getItsMove();
            if(p_Move != NULL)
                {
                    itsCanButton->__setItsMove(NULL);
                }
            itsCanButton = NULL;
        }
}

void Move_C::cancelTimeouts() {
    if(up_timeout != NULL)
        {
            up_timeout->cancel();
            up_timeout = NULL;
...
bool Move_C::cancelTimeout(const IOxfTimeout* arg) {
    bool res = false;
    if(up_timeout == arg)
        {
            up_timeout = NULL;
...
void Move_C::__setItsCanButton(CanButton_C* p_CanButton) {
    itsCanButton = p_CanButton;
}

void Move_C::_setItsCanButton(CanButton_C* p_CanButton) {
    if(itsCanButton != NULL)
        {
            itsCanButton->__setItsMove(NULL);
        }
...
void Move_C::_clearItsCanButton() {
    itsCanButton = NULL;
}

void Move_C::rootState_entDef() {
    {
...
IOxfReactive::TakeEventStatus Move_C::rootState_processEvent() {
...
IOxfReactive::TakeEventStatus Move_C::up_handleEvent() {
    IOxfReactive::TakeEventStatus res = eventNotConsumed;
...
IOxfReactive::TakeEventStatus Move_C::check_handleEvent() {
    IOxfReactive::TakeEventStatus res = eventNotConsumed;
    if(IS_EVENT_TYPE_OF(evMove_CanButton_id))
        {
            up_subState = moving;
...
IOxfReactive::TakeEventStatus Move_C::down_handleEvent() {
    IOxfReactive::TakeEventStatus res = eventNotConsumed;
